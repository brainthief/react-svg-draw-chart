...
class Min5 extends Component {
  ...
  componentDidMount() {
    ...
    fetch(
      "https://www.alphavantage.co/query?function=FX_INTRADAY&from_symbol=EUR&to_symbol=USD&interval=60min&outputsize=full&apikey=3JM7MXP7AKYCXD45"
    )
      .then(r => r.json())
      .then(res => {
  ...
  render() {
    ...
    if (this.state.duomenys.length === undefined) {
      var a = [];
      for (var index in this.state.duomenys["Time Series FX (60min)"]) {
        // a = a["Time Series (5min)"][index];
        // a.push(index);
        a.push([
          index,
          this.state.duomenys["Time Series FX (60min)"][index]["1. open"],
          this.state.duomenys["Time Series FX (60min)"][index]["2. high"],
          this.state.duomenys["Time Series FX (60min)"][index]["3. low"],
          this.state.duomenys["Time Series FX (60min)"][index]["4. close"]
        ]);
