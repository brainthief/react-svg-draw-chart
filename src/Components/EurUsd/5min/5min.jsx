...
class Min5 extends Component {
  ...
  componentDidMount() {
    ...
    fetch(
      "https://www.alphavantage.co/query?function=FX_INTRADAY&from_symbol=EUR&to_symbol=USD&interval=5min&outputsize=full&apikey=3JM7MXP7AKYCXD45"
    )
      .then(r => r.json())
      .then(res => {
  ...
  render() {
    ...
    if (this.state.duomenys.length === undefined) {
      var a = [];
      for (var index in this.state.duomenys["Time Series FX (5min)"]) {
