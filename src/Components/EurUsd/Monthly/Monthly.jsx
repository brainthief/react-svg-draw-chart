...
class Min5 extends Component {
  ...
  componentDidMount() {
    ...
    fetch(
      "https://www.alphavantage.co/query?function=FX_MONTHLY&from_symbol=EUR&to_symbol=USD&outputsize=full&apikey=3JM7MXP7AKYCXD45"
    )
      .then(r => r.json())
      .then(res => {
  ...
  render() {
    ...
    if (this.state.duomenys.length === undefined) {
      var a = [];
      for (var index in this.state.duomenys["Time Series FX (Monthly)"]) {
        // a = a["Time Series (5min)"][index];
        // a.push(index);
        a.push([
          index,
          this.state.duomenys["Time Series FX (Monthly)"][index]["1. open"],
          this.state.duomenys["Time Series FX (Monthly)"][index]["2. high"],
          this.state.duomenys["Time Series FX (Monthly)"][index]["3. low"],
          this.state.duomenys["Time Series FX (Monthly)"][index]["4. close"]
        ]);
